...
const team: TeamMember[] = [
  {
    ...
    description: () => (
      <>
        ...
        <p>Never communicates directly with the client but allows them to follow her process in Figma from the start. Handles projects together with <Button data-member={3} isLink className="underline decoration-dotted text-nowrap">Egor<Image className="inline ml-0.5 align-text-bottom w-[11px]" src="/images/model-1.png" alt="Egor 3d model" width={11} height={14} /></Button>.</p>
  ...
  },
  {
    ...
    description: () => (
      <>
        ...
        <p>Shares studio <Link href="#" className="underline">expectations</Link> in every project. Primary and often the only point of contact for projects involving <Button data-member={0} isLink className="underline decoration-dotted text-nowrap">Ksusha<Image className="inline ml-0.5 align-text-bottom w-[11px]" src="/images/model-1.png" alt="Ksusha 3d model" width={11} height={14} /></Button> and <Button data-member={2} isLink className="underline decoration-dotted text-nowrap">Lena R<Image className="inline ml-0.5 align-text-bottom w-[11px]" src="/images/model-1.png" alt="Lena R 3d model" width={11} height={14} /></Button>.</p>
      </>
    )
  },
  {
    ...
    description: () => (
      <>
        ...
        <p>Shares studio <Link href="#" className="underline">expectations</Link> in every project. Main point of contact for projects involving <Button data-member={1} isLink className="underline decoration-dotted text-nowrap">Lena S<Image className="inline ml-0.5 align-text-bottom w-[11px]" src="/images/model-1.png" alt="Lena S 3d model" width={11} height={14} /></Button>.</p>
      </>
    )
  }
]
...
export function Team() {
  ...
  useEffect(() => {
    ...
    const teamSection = sectionRef.current
    const teamDescription = descriptionsRef.current
    const startPosition = smoother?.offset(teamSection, 'center center') || 0
    ...
    const handleScrollTo = (e: MouseEvent) => {
      ...
      const memberOffset = team.reduce((prev, _, index) => {
        ...
        return section && index < memberIndex ? prev + section.clientWidth : prev
      }, 0);

      smoother.scrollTo(startPosition + memberOffset, true)
    }
        ...
        <div className="hidden md:flex flex-col items-start self-end">
          {team.map((member, index) => (
            <Button
              ...
              data-member={index}
              key={member.name}
              text={`${member.name.replace('.', '')}, ${member.role}`}
              blur={activeMemberIndex === index ? 'bold' : 'regular'}
            />
          ))}
        </div>
            ...
            <div key={member.name} className={`memberDescription ${team.length - 2 === index ? 'memberDescriptionEnd' : ''} flex justify-center lg:justify-start w-full md:w-[calc(42vw_-_20px)] lg:w-[calc(43.5vw_-_29px)] shrink-0`}>
              <div className="max-w-[264px] md:max-w-[180px] 2xl:max-w-[240px]">
                <h3 className="p1 uppercase -rotate-2">
                  <TextBlur isBold>{member.name}</TextBlur>
                </h3>
